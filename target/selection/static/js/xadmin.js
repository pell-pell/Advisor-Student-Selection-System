;!function (win) {
    ...
    //查看图片
    Xadmin.prototype.showCode=function(){
        layer.photos({
            photos: ".layui-table-main" //格式见API文档手册页
    ...
    Xadmin.prototype.add_lay_tab = function(title,url,id) {
        var rom=Math.floor(Math.random()*100000);
        //var url=url+"?v="+rom;
        var url=url;
     ...
     * [open 打开弹出层]
     * @param  {[type]}  title [弹出层标题]
     * @param  {[type]}  index   [弹出错误类型]
     * @return {[type]}        [description]
     */
    Xadmin.prototype.alert = function (title,index) {
        layer.alert(title,{"icon":index})
    }
     ...
     * @param param 请求参数
     */
    Xadmin.prototype.ajaxPostSumbit= function (url,param){
        if (url == null || url == '') {
            layer.alert("抱歉，请传入url地址，在进行提交",{icon:"2"});
            return false;
        }
        if (param == null || param == '') {
            layer.alert("抱歉，请传入参数，在进行提交",{icon:"2"});
            return false;
        }
        var index;
        $.ajax({
