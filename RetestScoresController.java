...
@Controller
@RequestMapping("/api/admin/retestscores")
public class RetestScoresController {


    /**
     * 路由地址
     */
    public String prefix = "/view/admin/retestscores";

    /**
     * 业务层
     */
    @Autowired
    private IRetestScoresService service;
    ...
    @RequestMapping("/list")
    @ResponseBody
    public Result getList(QPage qPage, RetestScoresView obj, HttpSession session) {
    ...
    @ResponseBody
    public Result insert(RetestScores obj,HttpSession session) {
        String role=(String)session.getAttribute("role");
        if(!role.equals("1")){//
    ...
    public Result getById(Integer id) {
        RetestScores dbo=this.service.getByIds(id);
    ...
    @ResponseBody
    public Result updates(RetestScores obj,HttpSession session) {
        String role=(String)session.getAttribute("role");
        if(!role.equals("1")){//
